...
from repomedic.models_commands import DependencyInfo, ExplainReport
...
def collect_explain(target: Path) -> ExplainReport:
    ...
    return ExplainReport(
        ...
        dependencies=[
            DependencyInfo(name=name, description=desc)
            for name, desc in _get_dependencies(target)
        ],
        file_count=len(files),
    )


def render_explain(report: ExplainReport, target: Path, out: Console | None = None) -> None:
    """Render collected explain data as rich panels/tables."""
    out = out or console
    langs = ", ".join(f"{name} ({count} files)" for name, count in report.languages.items()) or "none detected"
    out.print(Panel(
        f"[bold]Project Type:[/] {report.project_type}\n"
    ...
    if report.dependencies:
        ...
        for dep in report.dependencies:
            table.add_row(dep.name, dep.description)
...
def render_explain_markdown(report: ExplainReport) -> str:
    """Render the project brief as markdown — an agent onboarding document."""
    lines = [
        f"# Project Brief: `{report.target}`",
        "",
        f"- **Type:** {report.project_type}",
        f"- **Files:** {report.file_count}",
    ]
    langs = ", ".join(f"{name} ({count})" for name, count in report.languages.items())
